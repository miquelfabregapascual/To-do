...
If you have applied updates locally but do not see a pull request on GitHub yet, double-check that your commits have been pushed to
the remote repository:

1. Review local changes and commit them:
   ```bash
   git status
   git add <files>
...
2. Push the branch to GitHub and open a pull request:
   ...
   ```
   After the push completes, visit the repository in your browser to create a PR or confirm that an existing PR updated with the
   new commits.

This project uses the `work` branch for active development, so ensure that the branch exists on GitHub (`git push -u origin work`)
before expecting a pull request notification.
